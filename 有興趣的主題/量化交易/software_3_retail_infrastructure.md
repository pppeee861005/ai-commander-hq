...
| 層 | 量化交易（本專案） | 客服 agent | 法務 agent | 個人助理 agent |
|---|---|---|---|---|
| 大腦 | NotebookLM（研究、財報） | 案例庫 + 政策文件 RAG | 法條 + 判決書 RAG | 個人筆記 + 偏好庫 |
| 感知 | Futu API（行情、持倉） | CRM + 工單系統 API | 案件管理系統 API | 行事曆 + 郵件 API |
| 執行 | 模擬下單 + 通知 | 回覆草稿 + 升級工單 | 起草訴狀 + 排程開庭 | 安排會議 + 回信 |

每一欄都是一個可以做成 Skill 的垂直。每一欄的大腦、感知、執行都可以替換成不同的具體技術。但骨架是同一個。

這就是為什麼我們花這麼多力氣在量化這個垂直裡跑通樣板——不是因為我們想做一個量化系統，是因為我們想做一個**可以複製的樣板**。
...
這些不是「**功能**」，是**架構決策**。系統的存在不能侵蝕指揮官隨時離線的權利。
...
這些不是免責聲明，是**架構承諾**——我們承諾這個 Skill 不偽裝成它不是的東西。
...
*戰略層：Claude Opus 4.7 ｜ 執行層：Claude Code、Hermes Agent、未來會加入的富途 OpenAPI*
*指揮官：豪力風神*
